define(["knockout", "text!./home.html"], function(ko, homeTemplate) {
    ...
    function JsRpNEngine(lineOutputMethod, charOuputMethod) {
        var self = this;
...
//        defcode("RSP!", 0, function () { /* TODO */ });
    ...
    function HomeViewModel(route) {
        var self = this;
        var lineBuffer = ko.observableArray([]);
        self.cmdInput = ko.observable("");

        for (var i = 0; i < 25; i++) {
            lineBuffer.push('');
        }

        var AddLineToDisplay = function(line) {
            lineBuffer.shift();
            lineBuffer.push(line);
        };

        var jsRpNEngine = new JsRpNEngine(AddLineToDisplay);

        self.cmdInput.subscribe(function(newVal) {
            if (newVal.length !== 0) {
                jsRpNEngine.run(newVal);
                AddLineToDisplay(newVal);
                AddLineToDisplay(' => ' + jsRpNEngine.getStack() + ' => ' + jsRpNEngine.getReturnStack());
                self.cmdInput('');
            }
        });


        this.displayBuffer = ko.pureComputed(function () {
